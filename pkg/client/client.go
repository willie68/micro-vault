...
func (c *Client) CreateCertificate(template x509.CertificateRequest) (*x509.Certificate, error) {
	...
	if err != nil {
		logging.Root.Errorf(errMsgKeyFailed, err)
		return nil, err
	}
	defer res.Body.Close()
	if res.StatusCode != http.StatusCreated {
		logging.Root.Errorf(errMsgKeyBadRes, res.StatusCode)
		return nil, ReadErr(res)
	}
	b, err := io.ReadAll(res.Body)
	if err != nil {
		logging.Root.Errorf(errMsgHexConvertFailed, err)
		return nil, err
	}
...
func (c *Client) Encrypt4Group(g, dt string) (string, string, error) {
	...
	if err != nil {
		logging.Root.Errorf("key creation failed: %v", err)
		return "", "", err
	}
	...
	if err != nil {
		logging.Root.Errorf("reconstruct cipher failed: %v", err)
		return "", "", err
	}
	return cs, jr.ID, err
}
...
func (c *Client) StoreDataSS(n string, p string) (string, error) {
	...
	if err != nil {
		logging.Root.Errorf(errMsgReqFailed, err)
		return "", err
	}
	if res.StatusCode != http.StatusCreated {
		logging.Root.Errorf(errMsgMsgBadRes, res.StatusCode)
		return "", ReadErr(res)
	}
	var id struct {
		ID string `json:"id"`
	...
	err = ReadJSON(res, &id)
	if err != nil {
		logging.Root.Errorf(errMsgJSONFailed, err)
		return "", err
	}
	return id.ID, nil
}
...
func (c *Client) Encrypt4Client(n, dt string) (string, error) {
	...
	if err != nil {
		logging.Root.Errorf("encryption failed: %v", err)
		return "", err
	}
	return cs, err
}
...
func (c *Client) HMAC256Verify(msg pmodel.SignMessage) (bool, error) {
	...
	if err != nil {
		logging.Root.Errorf("key reading failed: %v", err)
		return false, err
	}
	b, err := hex.DecodeString(jr.Key)
	if err != nil {
...
func (c *Client) Sign(dt string) (*pmodel.SignMessage, error) {
	...
	err = ReadJSON(res, &sm)
	if err != nil {
		logging.Root.Errorf("read signature failed: %v", err)
		return nil, err
	}
	return &sm, nil
}
